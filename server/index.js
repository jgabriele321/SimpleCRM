...
const partnerStats = (deals) => {
    ...
    for (const d of deals) {
        ...
        if (pid == null)
            continue;
        if (!s[pid])
            s[pid] = { dealCount: 0, wonCount: 0, openCount: 0, totalValue: 0, wonValue: 0 };
        const st = normalizeStage(d.stage);
        s[pid].dealCount++;
        s[pid].totalValue += d.expectedValue || 0;
        if (st === 'closed_won') {
            s[pid].wonCount++;
            s[pid].wonValue += d.expectedValue || 0;
        }
